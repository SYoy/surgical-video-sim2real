...
class MUNIT_Trainer(nn.Module):
    ...
    def get_reprojected_view(self, renderdata, renderdata_2, scene_idx):
        with torch.no_grad():
            # synthesize 2nd view
            ...
            # reproject
            x_r, z_r = self.renderer.reproject(projectionMatrix,points3D_2,x)
            z = self.renderer.get_z(projectionMatrix,points3D)
            x_r = self.renderer.remove_occlusions(x_r,z_r,z)
        return x_r.detach(), x.detach()

    def reprojection_error(self,img,reprojected):

        reprojected = reprojected.detach()
        
        mask = (reprojected != 0)
        mask,_ = mask.min(dim=1)
        num_reprojected_pixels = mask.sum()
        if num_reprojected_pixels == 0:
            num_reprojected_pixels = 1

        img, reprojected = (img+1)/2, (reprojected+1)/2

        error = self.ang_dis(img,reprojected)
        error = torch.where(mask, error, torch.cuda.FloatTensor([0]))
        return error.sum() / num_reprojected_pixels
    ...
    def gen_update(self, x_a, x_b, hyperparameters, useLabelLoss=False):
        ...
        # reprojection loss
        if hyperparameters['use_reprojection_loss']:
            x_r, x_ab_2 = self.get_reprojected_view(renderdata, renderdata_2, scene_idx)
            loss_reprojection = self.reprojection_error(x_ab,x_r)
            loss_gen_total += hyperparameters['reprojection_w']*loss_reprojection
            self.loss_reprojection = loss_reprojection.item()
